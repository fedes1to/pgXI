...
namespace Rilisoft
{
	internal sealed class DigestStorager
	{
		...
		public bool ContainsKey(string key)
		{
			...
			return (!_useCryptoPlayerPrefs) ? PlayerPrefs.HasKey(backingStoreKey) : CryptoPlayerPrefsFacade.HasKey(backingStoreKey);
		}
		...
		public bool VerifyCore(string key, byte[] valueBytes)
		{
			...
			string backingStoreKey = GetBackingStoreKey(key);
			string s = ((!_useCryptoPlayerPrefs) ? PlayerPrefs.GetString(backingStoreKey) : CryptoPlayerPrefsFacade.GetString(backingStoreKey));
