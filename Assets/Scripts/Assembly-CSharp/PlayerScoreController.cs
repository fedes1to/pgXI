...
public class PlayerScoreController : MonoBehaviour
{
	...
	private void Start()
	{
		if (Defs.isMulti && ((Defs.isInet && !GetComponent<PhotonView>().isMine) || (!Defs.isInet && !GetComponent<PhotonView>().isMine)))
		{
			base.enabled = false;
			return;
		}
	...
	public void AddScoreOnEvent(PlayerEventScoreController.ScoreEvent _event, float _koef = 1f)
	{
		...
		if (num != 0)
		{
			...
			currentScore = Mathf.Max(0, num + currentScore);
			string text = PlayerEventScoreController.messageOnEvent[_event.ToString()];
			if (!string.IsNullOrEmpty(text))
			{
				AddScoreMessage(((num <= 0) ? string.Empty : "+") + num + " " + LocalizationStore.Get(text), num);
			}
		}
