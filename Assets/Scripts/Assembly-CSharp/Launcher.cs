...
internal sealed class Launcher : MonoBehaviour
{
	...
	[CompilerGenerated]
	private sealed class _003CAppsMenuStartCoroutine_003Ec__Iterator186 : IDisposable, IEnumerator<float>, IEnumerator
	{
		...
		{
			//Discarded unreachable code: IL_032f
		...
		if (!_progress.HasValue)
		{
			...
			foreach (float item3 in InitRootCoroutine())
			{
				...
				if (stopwatch != null)
				{
					...
					if (step2 >= 0f && elapsedSeconds < 1.618f / (float)_targetFramerate)
					{
						continue;
					}
				}
	...
	private IEnumerable<float> InitRootCoroutine()
	{
		...
		if (switcher != null)
		{
			...
			foreach (float item3 in switcher.InitializeSwitcher(null))
			{
				float step2 = item3;
		...
		foreach (float item4 in switcher.LoadMainMenu(false))
		{
			float step = item4;
