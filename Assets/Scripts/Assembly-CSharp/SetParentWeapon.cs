...
internal sealed class SetParentWeapon : MonoBehaviour
{
	...
	{
		int num = -1;
		PhotonPlayer owner = GetComponent<PhotonView>().owner;
		...
		{
			if ((!isInet || !(player.mySkinName.photonView != null) || player.mySkinName.photonView.owner == null || player.mySkinName.photonView.owner.ID != num) && (isInet || !player.mySkinName.GetComponent<PhotonView>().owner.Equals(owner)))
			{
				continue;
			}
