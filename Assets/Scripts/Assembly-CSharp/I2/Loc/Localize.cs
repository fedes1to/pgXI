...
namespace I2.Loc
{
	...
	public class Localize : MonoBehaviour
	{
		...
		private Texture mTarget_GUITexture;
		...
		{
			if (!mTarget_GUITexture || !mTarget_GUITexture)
			{
				SetFinalTerms(string.Empty, string.Empty, out PrimaryTerm, out SecondaryTerm);
				return false;
			}
			return SetFinalTerms(mTarget_GUITexture.name, string.Empty, out PrimaryTerm, out SecondaryTerm);
		}
		...
		{
			Texture texture = mTarget_GUITexture;
			if (!texture || !(texture.name == MainTranslation))
			{
				mTarget_GUITexture = FindTranslatedObject<Texture>(MainTranslation);
			}
		}
